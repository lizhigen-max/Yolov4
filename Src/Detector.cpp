...
void show_bbox(PredictMode mode, cv::Mat& image, bool show, const std::string& savePath, 
	torch::Tensor bboxes, std::vector<std::string> name_list) {
	...
	case P_IMG: {
		if (!savePath.empty()) {
			std::string basePath = YoGlobal::GetFilePath(savePath);
			if (!does_exist(basePath)) {
				YoGlobal::MakeNDir(basePath);
			}
...
YOLO_RESULT Detector::Initialize(int gpu_id, int width, int height,std::string name_list_path) {
	CLEAR_STR(errStr);
	if (gpu_id >= 0) {
		if (gpu_id >= torch::getNumGPUs()) {
			char data[256];
			sprintf_s(data, "%s %d %s", "No GPU id", gpu_id, "abailable");
			errStr = data;
			return YO_FAIL;
		}
	...
	if (!ifs.is_open())
	{
		errStr = "Open " + name_list_path + " file failed.";
		return YO_ERROR;
	}
	std::string buf = "";
	...
	if (width % 32 || height % 32) {
		errStr = "Width or height is not divisible by 32";
		return YO_ERROR;
	}
...
void Detector::Train(std::string train_val_path, std::string image_type, int num_epochs, int batch_size,
	...
	for (int epoc_count = 0; epoc_count < num_epochs; epoc_count++) {
		...
		if (best_loss >= loss_val) {
			best_loss = loss_val;
			torch::save(detector, save_path + std::to_string(loss_train) + "_" + YoGlobal::GetTime() + ".pt");
		}
	}
}

YOLO_RESULT Detector::LoadWeight(std::string weight_path) {
	CLEAR_STR(errStr);
	try
	{
...
cv::Mat Detector::Predict(PredictMode mode, cv::Mat image, bool show, const std::string& savePath, float conf_thresh, float nms_thresh) {
	CLEAR_STR(errStr);
	...
	// permute({ 2, 0, 1 }) 因为 opencv读取的图片格式是HWC，tensor 格式是BCHW
	// unsqueeze(0) 在原有tensor第0维增加一个维度，添加B维
	...
	auto tensor_input = outputs[0]; // 1,18,13,13

	auto output_decoded = DecodeBox(tensor_input, anchors_.narrow(0, 3, 3), name_list.size(), image_size);
	output_list.push_back(output_decoded);

	std::cout << tensor_input << anchors_.narrow(0, 3, 3);

	tensor_input = outputs[1];  // 1,18,26,26
	output_decoded = DecodeBox(tensor_input, anchors_.narrow(0, 1, 3), name_list.size(), image_size);
	output_list.push_back(output_decoded);

	std::cout << tensor_input << anchors_.narrow(0, 1, 3);
	...
	if (detection[0].size(0) < 7) {
		// not found
		errStr = "No object under test detected.";
		return image;
	}
	...
	show_bbox(mode, img_copy, show, savePath, detection[0], name_list);
	return img_copy;
}
